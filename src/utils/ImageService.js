...
export const IMAGE_CONFIG = {
  ...
  qualityPresets: {
    ...
    thumbnail: 80, // Sharp for small previews
...
class ImageService {
  ...
  processImage(imageData, variant = 'gallery', options = {}) {
    ...
    // Process image
    const processedUrl = this.generateCloudinaryUrl(imageData, variant, options);
  ...
  getMetrics() {
    return {
      ...
      hitRate: this.performanceMetrics.cacheHits / Math.max(1, this.performanceMetrics.processed)
    };
  }
  ...
  processResponsiveImage(imageData, options = {}) {
    ...
    Object.entries(breakpoints).forEach(([breakpoint, variant]) => {
      responsiveUrls[breakpoint] = this.processImage(imageData, variant, options);
    });
